...
- **Sync workers** in `scripts/` read config (env + TypeScript), discover root and nested `.agents/plugins/`, and idempotently write:
  ...
  - `## packages (generated by mmaappss)` section in root `AGENTS.md` (Codex)
...
| `mmaappss-marketplaces-codex-sync.ts` | Surgically updates `## packages (generated by mmaappss)` section in root `AGENTS.md`. |
