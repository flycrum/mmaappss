...
`mmaappss.config.ts` at repo root or in package provides same enable/disable semantics. Use env, TS config, or both; env overrides TS config. Scripts run via `tsx` (e.g. `pnpm exec tsx .agents/plugins/mmaappss/scripts/mmaappss-marketplaces-claude-sync.ts`).
...
For "root context" (e.g. `AGENTS.md` at repo root) and "nested context" (e.g. `packages/pkg/AGENTS.md`): Cursor and Codex use `AGENTS.md` natively. Claude uses `CLAUDE.md`. mmaappss scripts symlink `CLAUDE.md` from `AGENTS.md` (recursively) so Claude sees the same instructions. All symlinked `CLAUDE.md` files go in `.gitignore`; only `AGENTS.md` is committed.
...
| `mmaappss-marketplaces-claude-sync.ts` | Writes `.claude-plugin/marketplace.json`, symlinks rules to `.claude/rules/`. |
