...
From a sync entrypoint (e.g. `mmaappss-sync-claude-sync.ts`) to output:

```mermaid
flowchart TB
  subgraph entry [Entry]
    Script["mmaappss-sync-claude-sync.ts"]
...
2. **Plugin-internal tooling** (mmaappss) — Sync workers, CLIs, build steps that run outside the agent session (e.g. `mmaappss-sync-claude-sync.ts`).
...
| Script | Purpose |
|--------|---------|
| `mmaappss-sync-all.ts` | Runs all marketplace syncs (Claude, Cursor, Codex). |
| `mmaappss-sync-claude-sync.ts` | Writes `.claude-plugin/marketplace.json`, symlinks rules to `.claude/rules/`, and symlinks `CLAUDE.md` from `AGENTS.md` (root and nested) when Claude is enabled. |
| `mmaappss-sync-cursor-sync.ts` | Syncs plugin content into `.cursor/`: rules as .mdc (with `alwaysApply: true`), commands/skills/agents symlinked. No marketplace.json (Cursor does not support local marketplaces). |
| `mmaappss-sync-codex-sync.ts` | Surgically updates `## packages (generated by mmaappss)` section in root [`AGENTS.override.md`](https://developers.openai.com/codex/guides/agents-md/). |

Run via package script: `pnpm run mmaappss:sync:claude:sync` (and equivalents). See [packages/sync/README.md](../../README.md) for package-level overview.
